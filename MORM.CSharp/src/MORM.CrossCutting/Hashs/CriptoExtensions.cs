...
namespace MORM.CrossCutting
{
    ...
    {
        private static string _chave = "aq2SW#de$fr%HY&*";

        public static string Encrypt(string value, string chave = null)
        {
            if (string.IsNullOrWhiteSpace(value))
                return null;

            var crip = new Criptografia(CryptProvider.DES);
            crip.Key = chave ?? _chave;
            var text = crip.Encrypt(value);
            crip = null;

            return text;
        }

        public static string Decrypt(string value, string chave = null)
        {
            if (string.IsNullOrWhiteSpace(value))
                return null;
